...
const Essentials: React.FC<EssentialsProps> = ({ onStartAssessment }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 sm:py-16 md:py-20 px-4">
      <div className="text-center mb-12 sm:mb-16">
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-3 sm:mb-4 italic px-2">
          The Essentials
        </h2>
        <p className="text-zinc-500 uppercase tracking-[0.2em] sm:tracking-[0.3em] text-[10px] sm:text-xs">
          Pure Plant Support
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 mb-16 sm:mb-20 md:mb-24">
        {products.map((p, i) => (
          <div
            key={i}
            className="glass-card p-6 sm:p-7 md:p-8 group hover:border-[#C5A059]/50 transition-all"
          >
            ...
            <p className="text-zinc-400 text-sm leading-relaxed mb-4 sm:mb-6">{p.desc}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
        {bundles.map((b, i) => (
          <div
            key={i}
            className="bg-zinc-900/50 border border-white/5 p-6 sm:p-7 md:p-8 relative overflow-hidden group"
          >
            ...
            <button className="w-full py-3 sm:py-4 border border-white/10 text-white text-xs font-bold uppercase tracking-widest hover:bg-white hover:text-black transition-all touch-manipulation min-h-[44px]">
              Learn More
            </button>
          </div>
        ))}
      ...
      <div className="mt-16 sm:mt-20 md:mt-24 p-8 sm:p-10 md:p-12 border border-[#C5A059]/30 text-center">
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 sm:mb-6 italic px-4">
          Not sure which one to pick?
        </h3>
        <button
          onClick={onStartAssessment}
          className="px-8 sm:px-10 md:px-12 py-4 sm:py-5 md:py-6 gold-bg text-black font-bold uppercase tracking-[0.2em] shadow-lg hover:scale-105 transition-transform touch-manipulation min-h-[48px] text-sm sm:text-base"
        >
